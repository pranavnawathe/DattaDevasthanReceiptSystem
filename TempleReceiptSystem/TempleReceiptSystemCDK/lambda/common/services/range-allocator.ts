...
import { Keys, RangeItem, RangeStatus } from '../types';
...
export class RangeAllocationError extends Error {
  constructor(
    message: string,
    public code: string,
 ...
 */
export async function getActiveRange(orgId: string, year: number): Promise<RangeItem | null> {
  const params = {
 ...
 */
export async function allocateFromRange(
  orgId: string,
  year: number,
  donationDate: string,
  ...
  if (activeRange.next > activeRange.end) {
    throw new RangeAllocationError(
      `Range ${activeRange.rangeId} is exhausted. All numbers (${activeRange.start}-${activeRange.end}) have been used.`,
      'RANGE_EXHAUSTED',
      {
  ...
  try {
    ...
    if (isNowExhausted) {
      ...
      updateParams.ExpressionAttributeValues[':exhausted'] = RangeStatus.EXHAUSTED;
    }

    const result = await docClient.send(new UpdateCommand(updateParams));
    const updatedRange = result.Attributes as RangeItem;

    // 5. Generate receipt number
    const receiptNo = generateReceiptNo(activeRange.year, currentSeq);
    ...
    if (error.name === 'ConditionalCheckFailedException') {
      ...
      const freshRange = await docClient.send(
        new GetCommand({
          TableName: getTableName(),
          Key: {
            PK: Keys.PK.org(orgId),
      ...
      const updatedRange = freshRange.Item as RangeItem;

      // Check if it's still active
      if (updatedRange.status !== RangeStatus.ACTIVE) {
        throw new RangeAllocationError(
 ...
 */
async function allocateFromRangeSingleRetry(
  orgId: string,
  range: RangeItem,
  donationDate: string,
  donationYear: number,
  ...
  const updateParams: any = {
    ...
    Key: {
      PK: Keys.PK.org(orgId),
      SK: Keys.SK.range(range.rangeId),
    },
    UpdateExpression: 'SET #next = :newNext, #version = #version + :inc, #updatedAt = :now',
    ExpressionAttributeNames: {
      '#next': 'next',
  ...
  try {
    const result = await docClient.send(new UpdateCommand(updateParams));
    const receiptNo = generateReceiptNo(range.year, currentSeq);
 ...
 */
export function hasAvailableNumbers(range: RangeItem): boolean {
 ...
 */
export function getRemainingInRange(range: RangeItem): number {
