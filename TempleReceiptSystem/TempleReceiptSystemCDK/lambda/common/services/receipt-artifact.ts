...
export async function createPDFReceipt(donation: DonationItem): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      const normalisedBreakup: Record<string, number> = {};
      ...
      const dharmikLabelMap: Record<string, string> = {
      ...
      const categoryRows: Array<{ label: string; key: string }> = [
 ...
 * @returns S3 key where receipt was stored
 */
export async function createAndUploadReceipt(donation: DonationItem): Promise<string> {
