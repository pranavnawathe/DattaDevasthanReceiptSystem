...
describe('hashPAN', () => {
    it('should generate consistent hash for same PAN', () => {
        ...
        const hash1 = (0, crypto_1.hashPAN)(pan);
        const hash2 = (0, crypto_1.hashPAN)(pan);
        expect(hash1).toBe(hash2);
    });
    it('should generate different hashes for different PANs', () => {
        const hash1 = (0, crypto_1.hashPAN)('ABCDE1234F');
        const hash2 = (0, crypto_1.hashPAN)('XYZDE5678F');
        expect(hash1).not.toBe(hash2);
    });
    it('should include prefix', () => {
        const hash = (0, crypto_1.hashPAN)('ABCDE1234F');
        expect(hash).toMatch(/^h:sha256:/);
    });
});
describe('maskPAN', () => {
    it('should mask middle 4 characters', () => {
        expect((0, crypto_1.maskPAN)('ABCDE1234F')).toBe('ABCDE****F');
    });
    it('should throw error for invalid PAN length', () => {
        expect(() => (0, crypto_1.maskPAN)('ABC')).toThrow('Invalid PAN length');
    });
});
describe('hashEmail', () => {
    it('should generate consistent hash for same email', () => {
        const email = 'test@example.com';
        const hash1 = (0, crypto_1.hashEmail)(email);
        const hash2 = (0, crypto_1.hashEmail)(email);
        expect(hash1).toBe(hash2);
    });
    it('should generate different hashes for different emails', () => {
        const hash1 = (0, crypto_1.hashEmail)('test1@example.com');
        const hash2 = (0, crypto_1.hashEmail)('test2@example.com');
        expect(hash1).not.toBe(hash2);
    });
});
describe('maskEmail', () => {
    it('should mask local part', () => {
        expect((0, crypto_1.maskEmail)('user@example.com')).toBe('u***@example.com');
    });
    it('should handle short local part', () => {
        expect((0, crypto_1.maskEmail)('u@example.com')).toBe('u@example.com');
    });
});
describe('maskPhone', () => {
    it('should mask middle digits', () => {
        expect((0, crypto_1.maskPhone)('+919876543210')).toBe('+91987XXXXX10');
    });
    it('should return as-is for non-Indian phone', () => {
        expect((0, crypto_1.maskPhone)('+15551234567')).toBe('+15551234567');
    });
});
describe('shortHash', () => {
    it('should return 12 character hash', () => {
        const hash = (0, crypto_1.shortHash)('test-value');
        expect(hash.length).toBe(12);
    });
    it('should be consistent', () => {
        const hash1 = (0, crypto_1.shortHash)('test');
        const hash2 = (0, crypto_1.shortHash)('test');
        expect(hash1).toBe(hash2);
    });
});
describe('sanitizeForLogs', () => {
    it('should mask PAN in strings', () => {
        ...
        const output = (0, crypto_1.sanitizeForLogs)(input);
