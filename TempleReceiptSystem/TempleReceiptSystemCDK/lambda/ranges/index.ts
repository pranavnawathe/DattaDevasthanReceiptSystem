...
async function listRanges(
  ...
  let ranges = (result.Items || []) as RangeItem[];
  ...
  const enrichedRanges = ranges.map((range) => ({
    ...range,
    remaining: getRemainingCount(range),
  }));
 ...
 * GET /ranges/{rangeId}
 */
async function getRange(rangeId: string): Promise<APIGatewayProxyResultV2> {
  ...
  const range = result.Item as RangeItem;
...
async function checkRangeOverlap(year: number, start: number, end: number): Promise<RangeItem | null> {
  ...
  for (const existing of yearRanges) {
    ...
    const hasOverlap =
      (start >= existing.start && start <= existing.end) || // New start overlaps
 ...
 */
async function getActiveRange(year: number): Promise<RangeItem | null> {
  const params = {
    TableName: TABLE_NAME,
    KeyConditionExpression: 'PK = :pk AND begins_with(SK, :sk)',
    ExpressionAttributeValues: {
 ...
 * POST /ranges
 */
async function createRange(body: string): Promise<APIGatewayProxyResultV2> {
  ...
  const rangeId = suffix ? generateRangeId(year, suffix) : generateRangeId(year, 'A');

  // Check if range already exists
  const existingRange = await docClient.send(
    new GetCommand({
      TableName: TABLE_NAME,
      Key: {
        PK: Keys.PK.org(ORG_ID),
  ...
  return json(201, {
    ...
    range: {
      ...newRange,
      remaining: getRemainingCount(newRange),
    },
...
async function updateRangeStatus(
  ...
  const range = result.Item as RangeItem;
  let newStatus: RangeStatus;
  const now = new Date().toISOString();

  // Determine new status based on action
  switch (action) {
    case 'activate':
      if (!canActivateRange(range)) {
        return json(400, {
          success: false,
      ...
      newStatus = RangeStatus.ACTIVE;
      break;

    case 'lock':
      if (!canLockRange(range)) {
        return json(400, {
          success: false,
      ...
      newStatus = RangeStatus.LOCKED;
      break;

    case 'unlock':
      if (!canUnlockRange(range)) {
        return json(400, {
          success: false,
    ...
    case 'archive':
      newStatus = RangeStatus.ARCHIVED;
      break;
  ...
  try {
    ...
    const updatedRange = updateResult.Attributes as RangeItem;
    ...
    return json(200, {
      ...
      range: {
        ...updatedRange,
        remaining: getRemainingCount(updatedRange),
      },
